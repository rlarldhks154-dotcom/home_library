...
import io # 메모리 안에서 파일처럼 다룰 수 있도록 하는 라이브러리
...
class ServiceResult:
    ...
    status: str --> 결과 상태내는 문자열
        - ok : 정상 등록/조회 성공
        - invalid_isbn : isbn 체크성이 맞지 않는다(잘못 입력된 isbn)
        - duplicate : 이미 등록된 책
...
def lookup_book_service(isbn: str, db: Session) -> ServiceResult:
    ...
    book = Book(
        ...
        cover_path=None, # 표지 사진 없이 ISBN만으로 등록했기 때문에 이미지 경로가 없다.
...
def register_book_service(isbn: str, raw_image: bytes, original_filename: str, db: Session) -> ServiceResult:
    ...
    extension = Path(original_filename).suffix or '.jpg' # 이미지 확장자
    ...
    if metadata:
        ...
        # 서지 정보를 못 찾았어도 책 표지사진과 ISBN이 있다면 일단 등록한다. 수동 등록
    ...
    book = Book(
        ...
        author=author,
        publisher=publisher,
        cover_path=str(path), # DB는 Path객체 같은건 모른다. 문자열만 이해한다.
